...
public class UpdateConnection {
    ...
    private static Logger log = Logger.getLogger(UpdateConnection.class);
    ...
    public HashMap process(HashMap input) throws ParseException {
        ...
        String Status = dp.updateConnectionBiller(cbiller);
        input.put(FieldParameter.resp_code, Status);
        if (Status.equals("0000")) {
            dp.userYabes(input.get(FieldParameter.userlogin).toString(), "mengupdate connection " + input.get(FieldParameter.billercode).toString());
        }
        return input;
    }
}
