...
public class AddQr {
    ...
    private static Logger log = Logger.getLogger(AddQr.class);
...
//        qr.setMerchantbalance(input.get(FieldParameter.merchantbalance).toString());
        ...
        String status = dp.addQr(qr);
        input.put(FieldParameter.resp_code, status);
        if (status.equals("0000")) {
            dp.userYabes(input.get(FieldParameter.userlogin).toString(), "menambahkan Qr " + input.get(FieldParameter.merchantid).toString());
        }
        return input;
    }
}
