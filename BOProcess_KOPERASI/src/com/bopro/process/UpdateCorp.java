...
public class UpdateCorp {
    ...
    private static Logger log = Logger.getLogger(UpdateCorp.class);
    ...
    public HashMap process(HashMap input) throws ParseException {
        ...
        String status = dp.updateCorp(corp);
        input.put(FieldParameter.resp_code, status);
        if (status.equals("0000")) {
            dp.userYabes(input.get(FieldParameter.userlogin).toString(), "mengupdate korporasi " + input.get(FieldParameter.cu_id).toString());
        }
        return input;
    }
}
