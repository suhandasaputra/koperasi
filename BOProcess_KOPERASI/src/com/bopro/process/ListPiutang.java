 ...
 * @author herrysuganda
 */
public class ListPiutang {

    private static Logger log = Logger.getLogger(ListPiutang.class);

    public HashMap process(HashMap input) throws ParseException {
        BackendDBProcess dp = new BackendDBProcess();
        ArrayList<Settlement> ListSettlement = dp.getAllPiutang();
        Gson gson = new Gson();
        JsonElement element = gson.toJsonTree(ListSettlement, new TypeToken<List<Reportyabes>>() {
        }.getType());
        input.put(FieldParameter.resp_code, "0000");
        input.put(BackendParameter.listPiutang, element);
        return input;
    }
}
